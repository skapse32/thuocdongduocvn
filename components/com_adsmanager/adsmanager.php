...
function save_ad($option){
	...
	{
		$status = savePaidAd($row->id,$row->userid,$isUpdateMode,$errormsg);
		if ($status == -1)
		{
			return;
		}
...
function front($option) {
	...
	for($i=0;$i<count($tree[0]);$i++)
	{
		// get last 5 ads for each categories 
		$database->setQuery("SELECT * FROM #__adsmanager_ads a WHERE a.published=1 AND a.category='{$tree[0][$i]->id}'
		ORDER BY a.date_created DESC ,a.id DESC LIMIT 0, 5");
		$tree[0][$i]->ads= $database->loadObjectList();
		// get total ads for each categories 
		$database->setQuery("SELECT COUNT(*) FROM #__adsmanager_ads a WHERE a.published=1 AND a.category='{$tree[0][$i]->id}'");
		$tree[0][$i]->totalAds= $database->loadResult();
	}
...
function show_expiration($adid,$option) {
	...
	//TODO manage recall and credits
	if (!file_exists( $mosConfig_absolute_path . "/components/com_paidsystem/api.paidsystem.php"))
	{
