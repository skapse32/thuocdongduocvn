...
class plgSystemOSOLCaptcha extends JPlugin
{
  ...
  <tr>
	...
	<input type=\"hidden\" name=\"osolCatchaTxtInst\" id=\"osolCatchaTxtInst\"  value=\"".$GLOBALS['totalCaptchas']."\"   /><br/>
					...
					<tr><td colspan='3' style=\"text-align:left\"><input style='width:115px' type=\"text\" name=\"osolCatchaTxt\" id=\"osolCatchaTxt{$GLOBALS['totalCaptchas']}\"  class=\"inputbox required validate-captcha\" />&nbsp;</td></tr>
  </tr>
...
");
	...
	function mailBotScoutResult()
	{
				...
				$message = "Following request from IP:{$_SERVER['REMOTE_ADDR']} returned a -ve result on botscout verification in ".JURI::current()."\r\n Get vars =".var_export($_GET,true)."\r\n POST vars =".var_export($_POST,true)."\r\n REQUEST vars =".var_export($_REQUEST,true);
				//echo $subject."\r\n".$message ;exit;
