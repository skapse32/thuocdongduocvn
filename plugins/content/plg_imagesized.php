...
class plgContentPlg_imagesized extends JPlugin
{
	...
	function onPrepareContent ( &$article, &$params, $limitstart=0) //onAfterDisplayContent onPrepareContent
	{
		...
		if(($plugin_enabled=='1') && ($option == 'com_content') ) {
			...
			$plg_entrytext = $article->fulltext; 
	...
	{
		$limit_exec_time =ini_get('max_execution_time'); 	
		...
		{		
			...
			$height_orig=$width_orig=0;			
			set_time_limit(3);
			//list($width_orig, $height_orig) = @getimagesize($src);				
			//set_time_limit($limit_exec_time);					
			if(!$this->checkRemoteFile($src))
				$src=$baseurl.$cache_path_http.'/'.'blank.png';			
			if(!$this->checkRemoteFile($src))	
			{
				set_time_limit($limit_exec_time);
				return $text;
			...
			list($width_orig, $height_orig) = @getimagesize($src);		
			$image = $this->getimagedata($src);	
			if(!$image){set_time_limit($limit_exec_time); return $text;}
			$ratio_orig = $width_orig/$height_orig;
			if ($width/$height > $ratio_orig) {
			...
			if ($sample == false){set_time_limit($limit_exec_time); return $text;}
			$save = @imagejpeg($result, $full_path_filename, $quality);
			
			if ($save == false){set_time_limit($limit_exec_time); return $text;}
			@imagedestroy($image);
			@imagedestroy($result);
			$url = $cache_path_http."/".$filename;
		}
		$text = str_replace($src, $url, $text);
		//echo  time()-$_SESSION['xxx'].' seconds.  '."w=$widthm h=$heightm ";
		set_time_limit($limit_exec_time);
		return $text;
	}
