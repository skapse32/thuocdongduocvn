 ...
 * @since 1.5
 */
class ContentModelSectionHelper extends JModel
{
	/**
	 * Category id
	 *
	...
	function _buildQuery($state = 1)
	{
		...
		if(!empty($filter)&&$filter!='tvn')
		{
			...
			$oo ="a.title as oo";
		$query = 'SELECT '.$oo.', a.id, a.title, a.alias, a.title_alias, a.introtext, a.fulltext, a.sectionid, a.state, a.catid, a.created, a.created_by, a.created_by_alias, a.modified, a.modified_by,' .
	...
	function _buildContentWhere($state = 1)
	{
		...
		if (JRequest::getVar('filter','')!='')
		{
			...
			if ($filter) {
				...
				/*$where .= ' AND SUBSTRING(a.attribs,LOCATE('.$this->_db->Quote($filter).', a.attribs ),
						LOCATE('.$this->_db->Quote("\n").', a.attribs,LOCATE('.$this->_db->Quote($filter).', a.attribs ) )- 
						LOCATE('.$this->_db->Quote($filter).', a.attribs ))
				*/
