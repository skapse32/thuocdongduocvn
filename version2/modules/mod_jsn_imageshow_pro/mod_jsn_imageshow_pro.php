...
//Check if exist content id. get image from content
if($contentid){
	...
	$db=JFactory::getDBO();	
		...
		if(!empty($ids)){
			...
			$xml_data .= '<slideshow path="">'."\n";
			//sort($files);
			foreach ($Article as $aArticle) {				
				foreach($aArticle['photos'] as $aPhoto){					
						$xml_data .= "<image>".$aPhoto."</image>\n";					
				}
			}
...
// Begin output
echo '<div id="moduel_anhphongsu_title">
