...
class modSub_SlideHelper
{
	function getNews($maxnews=6)
	{
		$db=JFactory::getDBO();	
		...
		$where		= 'c.state = 1'
			...
			;
				...
				$db->setQuery("SELECT c.id,c.title,c.introtext,c.fulltext,c.catid,c.sectionid,c.attribs, 
				...
				WHERE  p.pos2='1' AND $where ORDER BY publish_up DESC LIMIT $maxnews");
