...
class plgContentExtranews extends JPlugin
{
	...
	function onAfterDisplayContent ( &$article, &$params, $limitstart ) //onAfterDisplayContent onPrepareContent
	{
		...
		if($plugin_enabled=="1" && $option=="com_content" && $view=="article"){
			...
			*****************/
			...
			// Content Items only
			$where_ids_related = "";
			$content = '<div id="inside_article"></div><br/><p id="link-back" class="link-back">
			...
			if (count( $likes ) && ($relateditemscount>0)) {
				...
				if (count($rows)) {
					$content .= '<h1 class="color5">
							BÀI LIÊN QUAN:</h1>';
					$content.="<ul class='list4'>";
			...
			if ($neweritemscount>0) {
				...
				*/
				...
				if (count($rows)){
					$content .= '<div id="neweritemtitle">'. JText::_('LBL_NEWERNAME').'</div><ul id="neweritemlist">'."\n";  
					...
					for ($i = 0; $i < $num_items; $i++ ) {
						...
						if($afirst==0){
							...
							$link   = JRoute::_(ContentHelperRoute::getArticleRoute($row->slug, $row->catslug, $row->sectionid.":testset"));
							$apre_next = '<span class="extranews_priviuospage"><a href="'. $link . '" title="'.$row->title.'">'.JText::_('LBL_PREV_PAGE').'</a></span>';
							$afirst = 0;
						}
			...
			$article->text .= $this->HTMLrender($article);			
	...
	{
		$params=$this->params;
		if(!$params->get("show_view_date",0))
			return '';		
		ob_start();
		...
		<input type="hidden" name="id" value="<?php echo $article->catid;?>?>"/>
		<input type="hidden" name="Itemid" value="<?php echo JRequest::getVar('Itemid');?>"/>	
		<h3 class="color5 viewdate">Xem theo ngày: 		
			<select name="day">
				<option value="<?php echo $curDay;?>">Ngày</option>
				<?php for($i=1;$i<=31;$i++):?>
					<option <?php// echo $i==$curDay?'selected':'';?> value="<?php echo $i;?>"><?php echo $i;?></option>
				<?php endfor;?>
			</select> 
			<select name="month">
				<option value="<?php echo $curMonth;?>">Tháng</option>
				<?php for($i=1;$i<=12;$i++):?>
					<option <?php// echo $i==$curMonth?'selected':'';?> value="<?php echo $i;?>"><?php echo $i;?></option>
				<?php endfor;?>
			</select> 
			<select name="year">
				<option value="<?php echo $curYear;?>">Năm</option>
				<?php for($i=$curYear;$i>=$lastTenYear;$i--):?>
					<option <?php// echo $i==$curYear?'selected':'';?> value="<?php echo $i;?>"><?php echo $i;?></option>
				<?php endfor;?>
			</select> 
		...
		ob_end_clean();
		$sections = $params->get('view_by_date_sections','');		
		$sections= explode(',',$sections);
		if(in_array($article->sectionid,$sections))
			return $content;
		return '';
