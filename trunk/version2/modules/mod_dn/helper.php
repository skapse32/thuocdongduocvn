<?php
###################################################################################################
#  Display News  1.5.9 - August-2008 by bkomraz1@gmail.com
   ...
   //---------------------------------------------------------------------
	...
	function dn_style_convert( $id ) {
		...
		$convert = array(
			...
			"dn-read_more" 			=> array("content" => "", 					"table" => "", 	                 "latestnews" => ""),
...
$version = "Display News by BK 1.5.7";
...
// $limit_title = $params->get('limit_title' , 0 );
...
$text_readmore 				= $params->get('text_readmore', JText::_('Read more...'));
...
($show_title ? ", a.title" : "" ).
(($show_text && !$show_full_text) ? ", a.introtext as text" : "" ).
...
// echo "\n <!-- A 'stingrey MOS-Solutions' module '".$website."' -->";
   ...
   if ( $show_title_auto ) {
      ...
      if ( $GLOBALS['_VERSION']->PRODUCT == 'Mambo' ) {
	      echo "<div ".modDisplayNewsHelper::dn_style("dn-title_auto").">";
	  } else {
		  echo "<h3>";
	  }
      echo $title_text;
      ...
      if ($show_category ) {
         ...
         if ($link_category ) {
            if ($category->published == "1") {
               // $category_out .= "\n\t\t\t";
               // $category_out .= "<a href='".JRoute::_("index.php?option=com_content&task=${blog}category&sectionid=$section->id&id=$category->id".$_Itemid."")."' ".modDisplayNewsHelper::dn_style("dn-category")." ";
...
//       if ($show_list== "y") {    $title_out .= "<li class='".$css_prefix."list-dn'>";  }
         if ($link_titles ) {
            // $title_out .= "<a href='".JRoute::_(ContentHelperRoute::getArticleRoute($row->id, $row->catid, $row->sectionid ))."' ".modDisplayNewsHelper::dn_style("dn-title")." ";
...
//         }
      ...
      if ($show_text ) {
		...
		if (!$show_image) {
			...
			$text = preg_replace( '/(<img\s+.*)(width\s*=\s*["]*\d+["]*)([^>]*>)/', '$1$3', $text );
			$text = preg_replace( '/(<img\s+.*)(height\s*=\s*["]*\d+["]*)([^>]*>)/', '$1$3', $text );
			$text = preg_replace( '/(<img\s+.*\s+style\s*=\s*")(\s*width:\s*\d+[px\s%]*;)([^>]*>)/', '$1$3', $text );
			$text = preg_replace( '/(<img\s+.*\s+style\s*=\s*")(\s*height:\s*\d+[px\s%]*;)(".*[^>]*>)/', '$1$3', $text );
			$text = preg_replace( '/(<img\s+.*src=[^ ]* )([^>]*>)/', '$1 '.($show_image == 2 ? "width" : "height").'='.$image_size.' $2', $text );
		}
        ...
        if ($filter_text ) {
         ...
         if ($link_text ) {
            // $intro_out .= "\n\t\t";
            // $intro_out .= "<a href='".JRoute::_("index.php?option=com_content&amp;task=view&amp;id=$row->id".$_Itemid."")."' ".modDisplayNewsHelper::dn_style("dn-introtext-link")." ";
      ...
      // $show_text && $show_full_text
        if ( ($show_readmore == 1) || 
         ...
         // $readmore_out .= "\n\t\t\t";
         // $readmore_out .= "<a href='".JRoute::_("index.php?option=com_content&amp;task=view&amp;id=$row->id".$_Itemid."")."' ".modDisplayNewsHelper::dn_style("dn-read_more")." ";
...
//      if ($show_arrow == "y") {  echo "<li ".modDisplayNewsHelper::dn_style("arrow-dn").">"; }
//      if ($show_list== "y")   {  echo "<li ".modDisplayNewsHelper::dn_style("list-dn").">";  }
...
//      } //if
	...
	//  echo "</div>"; // echo "<div ".modDisplayNewsHelper::dn_style("dn-whole").">";
   }
