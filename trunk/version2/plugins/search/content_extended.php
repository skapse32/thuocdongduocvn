...
function plgSearchContent( $text, $phrase='', $ordering='', $areas=null )
{
	...
	$plugin			=& JPluginHelper::getPlugin('search', 'content_extended');
	...
	//get back selected sections in search box
	$sectionids = $_SESSION['search.sectionid'];
	if(!empty($sectionids))
		$where =" (".$where.") AND a.sectionid IN(".$sectionids.")";
	$morder = '';
	switch ($ordering) {
		case 'oldest':
			$order = 'a.created ASC';
	...
	if ( $sContent && $limit > 0 )
	{
		$query = 'SELECT a.title AS title, a.metadesc, a.metakey,'
			...
			. ' CONCAT(a.introtext, a.fulltext) AS alltext,'
