...
class AttachmentsHelper
{
	...
	function upload_file(&$row, $article_id, $update=false, $attachment_id=false)
	{
		...
		// modified for accepting URL attachment
		$u_url=JRequest::getVar("upload_url","");
		if(empty($_FILES['upload']['name'])&&!empty($u_url))
		{			
			...
			$row->icon_filename = "download.gif";
	...
	function attachmentsTableHTML($article_id, $title, $show_file_links, $allow_modify, $from)
	{
		...
		if(!$loggedUser->id)
		{
			$html.="<tr><td colpan='3'>Đăng nhập trước khi tải file</td></tr>";
		}
		else
		{
		...
		for ($i=0, $n=count($rows); $i < $n; $i++) {
			...
			if ( $show_file_links ) {
				if ( $secure ) {
					...
					if(strpos(" ".$row->url,"http:")||strpos(" ".$row->url,"ftp:")||strpos(" ".$row->url,"https:"))					
						$url = $row->url;
					else
