...
class cf_multi_page  {
	...
	function onload( $option, $row, $params, $html_string ) {
		...
		$posted = array();
		if($session->get('chrono_formpages_data', array(), md5('chrono'))){
		...
		if($current_step == 1){
			$session->clear('chrono_formpages_data', md5('chrono'));
		}
	...
	function onsubmit( $option, $params , $row ) {
		...
		$oldposted = array();
		if($session->get('chrono_formpages_data', array(), md5('chrono'))){
			$oldposted = $session->get('chrono_formpages_data', array(), md5('chrono'));
		}
		$posted = array_merge($oldposted, $newposted);
		
		JRequest::set($posted, 'post');
		$session->set("chrono_formpages_data", $posted, md5('chrono'));
		$formname = JRequest::getVar('chronoformname');
		$MyForm =& CFChronoForm::getInstance($formname);
