...
class ContentViewCategory extends ContentView
{
	function display($tpl = null)
	{
		...
		if ($layout == 'blog' || $layout == 'blogcatalog') {
		...
		$sub_total	=	count($categories);
		$layout		=	( $layout ) ? $layout : 'default';
		foreach ( $categories as $sub_category  ) {
			//TODO: SEF URLS
			//$sub_category->link = JRoute::_( ContentHelperRoute::getCategoryRoute($sub_category->slug, $sub_category->section).'&layout='.$layout);
			$sub_category->link = JRoute::_( 'index.php?option=com_content&view=category&layout='.$layout.'&id='.$sub_category->id.'&Itemid='.$itemId );

			// Prepare category description
			$sub_category->description = JHTML::_('content.prepare', $sub_category->description);
		}
		if ($sub_total == 0) {
			$params->set('show_subcategories', 0);
		} else {
			if ( $params->get('show_articles') == 2 ) {
				$params->set('show_articles', 0);
			}
		}
	...
	function &getCat($index = 0, &$params)
	{
		global $mainframe;
		...
		$category_parent	= & $this->get( 'Category' );
		$cat->category		= $category_parent->title;
		$cat->section  		= $category_parent->sectiontitle;

		// Get the page/component configuration and article parameters
		$cat->params = clone($params);

		// Process the content preparation plugins
		JPluginHelper::importPlugin('content');
		$results = $dispatcher->trigger('onPrepareContent', array (& $cat, & $cat->params, 0));

		$cat->event = new stdClass();
		$results = $dispatcher->trigger('onAfterDisplayTitle', array (& $cat, & $cat->params,0));
		$cat->event->afterDisplayTitle = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onBeforeDisplayContent', array (& $cat, & $cat->params, 0));
		$cat->event->beforeDisplayContent = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onAfterDisplayContent', array (& $cat, & $cat->params, 0));
		$cat->event->afterDisplayContent = trim(implode("\n", $results));
