...
class adsmanager_html {
	...
	function selectCategories($id, $level, $children,&$catid,$root_allowed,$option,$itemid,$linkoption,$current_cat_only =0) {
		if (@$children[$id]) {
			foreach ($children[$id] as $row) {
				...
				adsmanager_html::selectCategories($row->id, $level.$row->name." >> ", $children,$catid,$root_allowed,$option,$itemid,$linkoption,$current_cat_only);
			}
		}
	}
					   ...
					   $fields_searchable)
	{
		...
		<div class="adsmanager_inner_box">
			...
			?>
			<div align="left"><input name="text_search" id="text_search" maxlength="20" alt="search" class="input" type="text" size="20" value="<?php echo $text_search;?>"  onblur="if(this.value=='') this.value='';" onfocus="if(this.value=='<?php echo $text_search;?>') this.value='';" /></div>
	...
	{		
		...
		function submitbutton(mfrm) {
			...
			if (mfrm.username && (r.exec(mfrm.username.value) || mfrm.username.value.length < 3)) {
				...
				//mfrm.username.style.background = "#FE7903";
				iserror=1;
			} 
			if (mfrm.password && r.exec(mfrm.password.value)) {
				...
				//mfrm.password.style.background = "#FE7903";
				iserror=1;
			}
			...
			if (mfrm.email && !r_email.exec(mfrm.email.value) && mfrm.email.getAttribute('mosReq')) {
				...
				//mfrm.email.style.background = "#FE7903";
				iserror=1;
			}
		...
		<fieldset id="adsmanager_fieldset">
			...
			<div id="adsmanager_writead_header" style='float:right'>
				<?php// if(ADSMANAGER_HEADER1!=''):?>
				<div id="writead_header1"><?php echo ADSMANAGER_HEADER1; ?></div>
				<?php// endif;?>
				<?php// if(ADSMANAGER_HEADER2!=''):?>
		  ...
		  <label for="ad_kindof"><?php echo ADSMANAGER_FORM_CATEGORY; ?></label>
		    <?php 
					  ...
					  file_exists( $mosConfig_absolute_path . "/components/com_paidsystem/api.paidsystem.php"))
				{
					...
					adsmanager_html::selectCategories(0,"",$cats,$catid,$conf->root_allowed,$option,
													  $itemid,$linkoption,$display_current_cat_only); 
				}
				else
				{
				?>
					...
					<?php
					 ...
					 adsmanager_html::selectCategories(0,"",$cats,$catid,$conf->root_allowed,$option,$itemid,$linkoption,0); 
					?>
					</select>
				<?
				}
				
		  if (($ad_id == 0)&&($catid != 0)&&(function_exists("checkBalance")))
		  {
			if (checkBalance($catid,$my->id) == -1)
				return;
		  }
		  ...
		  <form action="<?php echo $target;?>" method="post" name="saveForm" enctype="multipart/form-data" onsubmit="return submitbutton(this)">
			...
			if ((!isset($catid))||($catid != 0))
			{
				...
				if ($conf->nb_images > 0)
				{
					echo JText::sprintf(ADSMANAGER_FORM_AD_IMAGE_TEXT,$conf->nb_images,number_format($conf->max_image_size/1024,0));
					echo "<br />";
				}
	...
	function showProfile($user,$fields,$option,$itemid) {	
		...
		<?php echo "<div class=\"adsmanager_list_title\" width=\"100%\"><strong>".ADSMANAGER_EDIT_PROFILE."</strong></div>"; ?>
		<hr/>
		</div>
		<br />
		...
		<tr>
			...
			<td>
				<input class="input" type="password" name="password" value="" size="40" />
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<input class="input" type="password" name="verifyPass" size="40" />
			</td>
		</tr>
		<tr>
			<td colspan="2">
		...
		<tr>
			...
			<td>
				<input class="input" type="text" name="name" value="<?php echo $user->name;?>" size="40" />
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<input class="input" type="text" name="email" value="<?php echo $user->email;?>" size="40" />
			</td>
		</tr>
		<?php
		if (isset($fields)) {
		foreach($fields as $f)
		{
			...
			<tr>
				...
				<td>
					<input class="input" type="text" name="<?php echo $f->name;?>" value="<?php echo $value;?>" size="40" />
				</td>
			</tr>
			<?php }
		}
	...
	<div class='mdl-cnt'>
		...
		<div class='cnt'>
			<div style='padding-left:10px;'>
			<!--show links-->
			<?php if(count($cat->ads)):?>			
			<ul class='list_ads'>
			<?php foreach($cat->ads as $item):?>
			<?php
			// formated date
			...
			<?php else:?>
				<center><p><?php echo ADSMANAGER_NOENTRIES;?></p></center></br>
			<?php endif;?>
			&nbsp;</br>
	<?php echo adsmanager_html::show_footer();?>
	<?php endforeach;?>
		...
		<tr>
			<td align="center" width="400px">
				...
				<tr>
					...
					<input name="username" type="text" class="input" style='width:130px;' size="15" />
					</td>
					<td align="right" width='180'>
					<input name="passwd" type="password" class="input" style='width:130px;' size="15" />
					</td>
				...
				<tr>
					...
					<input type="checkbox" name="remember" class="input" value="yes" />
					<br />
